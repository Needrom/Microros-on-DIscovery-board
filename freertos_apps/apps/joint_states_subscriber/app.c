...
void subscription_joint_state_callback(const void *msgin){
	...
	printf("I get joint_state topic \r\n");
}
...
int appMain(void *argument)
{
	...
	RCCHECK(rclc_executor_add_subscription(&executor, &joint_states_subscriber, &joint_states_msg, &subscription_callback, ON_NEW_DATA));
